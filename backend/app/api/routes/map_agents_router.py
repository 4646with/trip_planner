...
async def plan_trip(request: TripRequest):
    ...
    """
    print(f"\n{'=' * 60}")
    print(f"[RECEIVED] POST /api/map-agents/plan")
...
@router.get("/health", summary="健康检查", description="检查地图智能体服务是否正常")
async def health_check():
    ...
    try:
        ...
        return {"status": "unhealthy", "service": "map-agents", "error": str(e)}
...
async def initialize_mcp():
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"MCP工具初始化失败: {str(e)}")


@router.post("/cleanup", summary="清理MCP资源", description="清理MCP服务器进程和资源")
async def cleanup_mcp():
    """手动清理 MCP 资源"""
    try:
        await cleanup_map_agents_system()
        return {"status": "success", "message": "MCP资源已清理"}
    except Exception as e:
        raise HTTPException(status_code=500, detail=f"MCP资源清理失败: {str(e)}")
