...
def create_tool_wrapper(mcp_tool: BaseTool, args_schema: BaseModel, tool_name: str) -> StructuredTool:
    """
...
class RateLimitedMCPToolsManager(MCPToolsManager):
    ...
    async def initialize(self) -> None:
        ...
        if self._session and not hasattr(self, '_original_call_tool'):
            ...
            async def safe_call_tool(name, arguments=None, **kwargs):
                ...
                async with self._semaphore:
                    ...
                    for attempt in range(max_retries):
                        try:
                            result = await self._original_call_tool(name, arguments, **kwargs)
                            ...
                            if getattr(result, "isError", False):
                                ...
                                if "CUQPS_HAS_EXCEEDED_THE_LIMIT" in error_text or "TOO_FAST" in error_text:
                                    logger.warning(f"⚠️ 高德 API 并发超限，{retry_delay}秒后重试 (尝试 {attempt+1}/{max_retries})...")
                            ...
                            if "CUQPS" in str(e) or "TOO_FAST" in str(e):
                                logger.warning(f"⚠️ 高德 API 调用超限，{retry_delay}秒后重试...")
                                await asyncio.sleep(retry_delay)
                                retry_delay *= 2
                                continue
            ...
            logger.info("✅ 已通过底层拦截为 MCP 会话添加全局并发控制和重试机制")


_mcp_manager: Optional[RateLimitedMCPToolsManager] = None


def get_mcp_manager() -> RateLimitedMCPToolsManager:
    """获取 MCP 工具管理器单例"""
    global _mcp_manager
    if _mcp_manager is None:
        _mcp_manager = RateLimitedMCPToolsManager()
    return _mcp_manager


async def initialize_mcp_tools() -> RateLimitedMCPToolsManager:
    """初始化 MCP 工具"""
